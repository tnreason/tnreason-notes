...
\begin{definition}[\ActivationCircuit{}]
    ...
    $\qcaencodingofat{\exformula}{\avariableof{\insymbol},\avariableof{\outsymbol},\shortcatvariables}$ defined as
    \begin{align*} %% Could ease the definition by coordinates
        \qcaencodingofat{\hypercore}{\avariableof{\insymbol},\avariableof{\outsymbol},\shortcatvariables}
        = \sum_{\shortcatindicesin} \onehotmapofat{\shortcatindices}{\shortcatvariables}
        \otimes \yrotationofat{h(\exformulaat{\shortcatindices})}{\avariableof{\insymbol},\avariableof{\outsymbol}} \, .
    \end{align*}
\end{definition}
...
\begin{theorem}[\cite{low_quantum_2014}]
    ...
    \end{align*}
    respecting the order $[\catorder]$ prepares the Bayesian Network $\probwith$ when acting on the initial state $\bigotimes_{\atomenumeratorin}\onehotmapofat{0}{\catvariableof{\catenumerator,\insymbol}}$.
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        ...
        \left(\qcaencodingofat{
            \exfunctionof{\catenumerator}
        }{\catvariableof{\catenumerator,\insymbol}=0,\indexedcatvariableof{\catenumerator},\indexedcatvariableof{\parentsof{\catenumerator}}}\right)^2 \\
        &= \prod_{\catenumeratorin}
...
When drawing a sample $(\shortcatindices,\aindex)$ from $\anaugmentationofat{\probtensor}{\shortcatvariables,\avariable}$ and rejecting it whenever $\aindex=0$, the accepted samples are distributed as $\probwith$.
We then have $\acceptanceprob$ as the probability of accepting a sampling in this rejection sampling method.
The acceptance probability is critical in determining the efficiency of this approach, since we need to draw $\sim\frac{1}{\acceptanceprob}$ samples from $\anaugmentationofat{\probtensor}{\shortcatvariables,\avariable}$ to get an accepted sample.
...
\begin{lemma}
    ...
    \end{align*}
    prepares a quantum state $\qstate$, which is the Q-sample of the ancilla augmentation of $\probtensor$
    \begin{align*}
...
\end{lemma}
\begin{proof}
    We first show, that $\anaugmentationof{\probtensor}$ is indeed an ancilla augmentation.
    We have
    \begin{align*}
        \acceptanceprob
        &= \contractionof{\anaugmentationofat{\probtensor}{\avariable,\shortcatvariables}}{\avariable=1} \\
        &= \frac{1}{\prod_{\catenumeratorin}\catdimof{\catenumerator}}
        \sum_{\shortcatindicesin} \probat{\shortcatindices} \\
        &= \frac{1}{\prod_{\catenumeratorin}\catdimof{\catenumerator}} \, .
    \end{align*}
    and for any $\shortcatindicesin$ it holds that
    \begin{align*}
        \anaugmentationofat{\probtensor}{\indexedshortcatvariables|\avariable=1}
        = \frac{1}{\lambda} \frac{1}{\prod_{\catenumeratorin}\catdimof{\catenumerator}} \probat{\indexedshortcatvariables}
        = \probat{\indexedshortcatvariables}\, ,
    \end{align*}
    The tensor $\anaugmentationofat{\probtensor}{\shortcatvariables,\avariable}$ is thus an ancilla augmentation for $\probtensor$.

    We continue to show that the quantum state $\qstate$ prepared by the \activationCircuit{} is the Q-sample of $\anaugmentationof{\probtensor}$.
    ...
    \end{align*}
    which is the Q-sample of $\anaugmentationof{\probtensor}$.
\end{proof}
...
\begin{align*}
    \acceptanceprob = \frac{1}{\prod_{\catenumeratorin}\catdimof{\catenumerator}} \probat{\indexedshortcatvariables} \, .
\end{align*}
...
We then build directed tensors
\begin{align*}
    \anaugmentationofat{\hypercoreof{\edge}}{\avariable,\catvariableof{\edge}}
    =     \frac{1}{\prod_{\catenumeratorin}\catdimof{\catenumerator}}
    ...
    } \cdot \tbasisat{\avariable} \\
    &+ (1- \frac{
        \hypercoreofat{\edge}{\indexedcatvariableof{\edge}}
    }{
        \max_{\seccatindexof{\edge}}\hypercoreofat{\edge}{\catvariableof{\edge}=\seccatindexof{\edge}}
    }
    ) \cdot \fbasisat{\avariable}
    \Big) \, .
\end{align*}
It is easy to check, that when augmenting all cores of a Markov Network with distinct ancilla variables in this way, we get an ancilla augmentation of the Markov Network.
...
When the maximal coordiante of the Markov Network is, restricted to each edge, the maximal coordinate of the hypercore of that edge, we say that the Markov Network is not frustrated, otherwise frustrated.

\begin{lemma}
    If the Markov Network is not frustrated, then the acceptance probability $\acceptanceprob$ of the rejection sampling scheme using the ancilla augmented Markov Network is the $\infty$-Renyi divergence between the Markov Network and the uniform distribution.
\end{lemma}
