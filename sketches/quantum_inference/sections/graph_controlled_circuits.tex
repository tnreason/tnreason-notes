...
\begin{lemma}
    ...
    \end{align*}
    prepares a quantum state $\qstate$, which measurement distribution is the ancilla augmented distribution of $\probwith$.
\end{lemma}
\begin{proof}
...
\end{center}
for a normalization constant $\acceptanceprob$.
%Here the measurement distribution is normalized by the
We interpret $\acceptanceprob$ as the acceptance probability, which is the probability
\begin{align*}
    \acceptanceprob = \contractionof{\secprobat{\avariable,\shortcatvariables}}{\avariable=1}\in[0,1] \, .
\end{align*}
%% Background: Rejection Sampling
To provide background on this interpretation, let us consider a rejection sampling approach of the distribution $\probwith$, based on its ancilla augmentation $\secprobat{\shortcatvariables,\avariable}$.
...
Now, the acceptance probability is critical in determining the efficiency of this approach:
We need to draw $\sim\frac{1}{\acceptanceprob}$ samples from $\secprobat{\shortcatvariables,\avariable}$ to get an accepted sample.
...
%Note that our ancilla encoding scheme of Markov Networks encodes each local tensor
