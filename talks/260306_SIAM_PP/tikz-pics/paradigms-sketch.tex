...
\begin{tikzpicture}[yscale=0.7, xscale=0.7]
    ...
    \node[anchor=center] at (0,-0.5) {
        \sketchtextsize Decomposition of functions into neurons};

    \node[anchor=west] at (-7.5,-5.1) {\bf Probabilistic paradigm};
    \draw[dashed] (-8,-1) rectangle (2,-5.5);
    \node[anchor=west, align=left] at (-7.75,-3.5) {
        \sketchtextsize Independence  %\\[-3pt]
    ...
    \node[anchor=west, align=left] at (-7.75,-4.25) {
        \sketchtextsize Marginal distributions% \\[-3pt]
    ...
    \node[anchor=east, align=right] at (7.75,-4.25) {
        \sketchtextsize Entailment
    %    (\defref{def:logicalEntailment})
    };
