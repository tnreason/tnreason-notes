...
However, not every maximum entropy distribution is an exponential family member.
...
We further show that a maximum entropy distribution is an exponential family member if and only if its mean parameter is in the relative interior of the mean polytope.

\subsection{Representation as elementary \ComputationActivationNetworks{}}

\todo{Fusion with the definition in previous section: Resolve the base measure difference.}
...
\begin{definition}[Exponential Family]
    ...
    \end{align*}
    is called the exponential family with respect to $\sstat$.
\end{definition}
...
\begin{figure}[t]
    \begin{center}
        \input{tikz-pics/exp-family/expdist_unary_realizable}
    \end{center}
    \caption{Representation of a member of an exponential family by a \ComputationActivationNetwork{} with elementary activation.
    Here $\partitionfunction\in\rr$ is a normalization constant.}\label{fig:expdistUnaryRealizable}
\end{figure}

We will use the following well known property (see e.g. \cite{brown_fundamentals_1987}), that the mean parameters of the members of an exponential family are exactly the relative interior of the mean polytope.
...
One universal property of exponential families amongst others (see \cite{murphy_probabilistic_2023}) is that they maximize the entropy among those with the same mean vector.
...
\end{lemma}
\begin{proof}
    Let $\expdistof{\sstat,\canparam,\basemeasure}$ be an exponential family member with canonical parameter $\canparam$ and let $\meanparamwith$ be the corresponding mean parameter.
    ...
    Therefore, any feasible $\secprobtensor$ different from $\expdistof{\sstat,\canparam,\basemeasure}$ has less entropy than $\expdistof{\sstat,\canparam,\basemeasure}$ and $\expdistof{\sstat,\canparam,\basemeasure}$ is a maximum entropy distribution.
\end{proof}
