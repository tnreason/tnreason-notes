\begin{example}[Codeword Polytopes]
    % See Example~3.9 in \cite{wainwright_graphical_2008}
    Consider the codeword polytope with the vertices
    \begin{align*}
        \imset
        =\left\{\headindexof{[3]} \wcols \sum_{\catenumerator\in[3]} \headindexof{\catenumerator} \mod 2=0 \right\}
        = \left\{
              \coloredmatrixof{0 \\ 0 \\ 0}{\selvariable},
              \coloredmatrixof{0 \\ 1 \\ 1}{\selvariable},
              \coloredmatrixof{1 \\ 0 \\ 1}{\selvariable},
              \coloredmatrixof{1 \\ 1 \\ 0}{\selvariable}
%              \begin{pmatrix}
%                  0 \\ 0 \\ 0
...
%            \end{pmatrix}
    ...
    Such constraints are common in the construction of codewords using parity-check codes (see \cite{gallager_low-density_1963}).
    \begin{center}
    ...
    We can construct $\ttformat$ activation tensors with rank $2$, which calculate the Hamming distance to these vertices via hidden ranks.
