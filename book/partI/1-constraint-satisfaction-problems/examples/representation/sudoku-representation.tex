\begin{example}[Sudoku representation]\label{exa:sudokuRepresentation}
    We recall the Sudoku knowledge base \exaref{exa:sudokuFormula} and now aim to find efficient tensor network representation.

    First of all, we notice that the formulas $\woneoplus[\catvariableof{[\sudokunum^2]}]$ coincide with the exactly-one constraint.


    We apply the decomposition of that constraint and get, that the knowledge base $\sudokukbof{\sudokunum}$ of Sudoku rules is a tensor network of the $4\cdot \sudokunum^4$ constraint formulas using the $\sudokunum^2$-ary connective $\woneoplus$, and the evidence $\sudokustartevidence$ can be encoded by vectors $\tbasisat{\catvariableof{(r_0,r_1,c_0,c_1,i)}}$.
