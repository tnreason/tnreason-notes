\section{Syntactical decompositions}

When syntax is available for formulas, we find tensor network decompositions.
This is a first example of basis calculus.

\subsection{Encoding of Booleans}\label{sec:booleanEncoding}

Before investigating schemes for syntactical decompositions, we revisit the encoding of Booleans.
...
\subsubsection{Representation by Basis Vectors} % This is what Basis Calculus does! Refer to that here?
...
\begin{align*}
    ...
    \coloredmatrixof{0 & 1 \\ 1 & 0}{\headvariableof{\lnot},\catvariable} \, ,
\end{align*}
...
\subsection{Basis encodings}

\todo{Add entailment by basis calculus diagram.}
...
\subsubsection{Formula representation by \ComputationActivationNetworks{}}
...
\begin{figure}[t]
    \begin{center}
        \input{tikz/formula_bencoding.tex}
    \end{center}
...
Syntax provides a description of a formula as a circuit of connective formulas.
...
\begin{figure}[t]
    \begin{center}
        \input{tikz/unary_binary_composition.tex}
    \end{center}
...
\begin{definition}
    ...
    A syntactical circuit is a directed acyclic hypergraph $\graph=(\nodes,\edges)$ (see \defref{def:hypergraphs}) such that
    \begin{itemize}
    ...
    \end{itemize}
    We say that the syntactical circuit is single-rooted, if exactly one node $\secnode$ does not appear as an incoming node of a hyperedge.
...
%	We further call any syntactical circuit, which composition is a formula $\exformula$, a syntactical decomposition of $\exformula$.
\end{definition}
...
\begin{theorem}
    ...
    For any syntactical circuit $\graph$ with composition $\exformula$ we have
    \begin{align*}
%		\bencodingofat{\exformula}{\formulavar,\shortcatvariables}
...
\begin{figure}[t]
    \begin{center}
        \input{tikz/decomposition_example.tex}
    \end{center}
...
\subsection{Minterm and maxterm decompositions}\label{sec:termClauseDecomposition}
%\subsect{Syntactical Decomposition of Formulas}
...
\begin{definition}[Terms and Clauses]
    \label{def:clauses}
    Given a subset $\hardlegset\subset[\catorder]$ and $\headindexof{\hardlegset}\in\bigtimes_{\catenumerator\in\hardlegset}[2]$, the corresponding term is the formula
    \begin{align*}
        \termofat{\hardlegset}{\shortcatvariables}
        = \left(\bigwedge_{\catenumerator\in\hardlegset\wcols\headindexof{\catenumerator}=0}\lnot\atomicformulaofat{\catenumerator}{\catvariableof{\catenumerator}}\right)
        \land
        \left(\bigwedge_{\catenumerator\in\hardlegset\wcols\headindexof{\catenumerator}=1}\atomicformulaofat{\catenumerator}{\catvariableof{\catenumerator}}\right)
    \end{align*}
    and the corresponding clause is the formula
    \begin{align*}
        \clauseofat{\hardlegset}{\shortcatvariables}
        = \left(\bigvee_{\catenumerator\in\hardlegset\wcols\headindexof{\catenumerator}=0}\atomicformulaofat{\catenumerator}{\catvariableof{\catenumerator}}\right)
        \lor
        \left(\bigvee_{\catenumerator\in\hardlegset\wcols\headindexof{\catenumerator}=1}\lnot\atomicformulaofat{\catenumerator}{\catvariableof{\catenumerator}}\right)
    \end{align*}
    \begin{align*}
    ...
    \end{align*}
    We call the term a minterm and the clause a maxterm, if $\hardlegset = [\catorder]$.
\end{definition}

\todo{Use the $\hardparam$ parametrization instead of the nodes 0 1 parametrization in the following.}
...
\begin{theorem}
    ...
    \begin{align*}
        ...
        \left( \bigvee_{\hyperonecoordinates}
