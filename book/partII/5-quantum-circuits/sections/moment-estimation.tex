...
\begin{figure}
    \begin{center}
        \input{tikz/moment-estimation/ancilla-grover}
    \end{center}
...
\todo{We here do Quantum Phase Estimation on a Grover operator to measure the moment of a function.}
...
\red{Instead of amplitude amplification we here exploit the Grover operator from a different perspective:
    \begin{itemize}
        \item Good and bad states by the restriction to the support of formulas
        \item Good state reflection using the oracle on an disentangled ancilla qubit in the Hadamard basis state $\hminusbasat{\headvariable}$
        \item We perform Quantum Phase Estimation to extract the moment through the phase of the eigenvalue of the Grover algorithm
    \end{itemize}
}
...
\subsection{Moments as rotation angles}
...
\subsection{Retrieving the moment from the phase}

%% Plane projection -> To groverOperator Section
...
The eigenvalues of the Grover operator are (see \secref{sec:groverOperator}) $\expof{\pm i\rotanglesymbol}$ and $1$.
%\end{align*}
%to the eigenstates
%\begin{align*}
%    \frac{1}{2}
