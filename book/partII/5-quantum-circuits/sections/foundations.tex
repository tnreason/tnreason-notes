...
\subsection{Quantum states and quantum circuits}

%First of all, we need to extend to complex tensors, which are maps
Quantum states are complex tensors
\begin{align*}
    \qstateat{\shortcatvariables} \defcols \atomstates \rightarrow \mathbb{C} \,
\end{align*}
such that
\begin{align*}
    \contraction{( \qstateat{\shortcatvariables})^*,\qstatewith}
    =\sum_{\shortcatindices\in\atomstates} ( \qstateat{\indexedshortcatvariables})^* \qstateat{\indexedshortcatvariables}
    =1 \, .
\end{align*}
Quantum gates are unitary tensors
\begin{align*}
    \unitaryat{\ccatvariable{\insymbol},\ccatvariable{\outsymbol}}
    \defcols \atomstates \times \atomstates \rightarrow \mathbb{C}
\end{align*}
that is
\begin{align*}
    \contractionof{(\unitaryat{\ccatvariable{\midsymbol},\ccatvariable{\outsymbol}})^\dagger, \unitaryat{\ccatvariable{\midsymbol},\ccatvariable{\insymbol}}}
    = \identityat{\ccatvariable{\insymbol},\ccatvariable{\outsymbol}}
\end{align*}
...
\begin{remark}
    Contractions of quantum circuits are understood on the semiring $(\mathbb{C},+,\cdot)$ instead of the default semiring $(\rr,+,\cdot)$.
\end{remark}
...
\begin{itemize}
    ...
    \item \textbf{Unitarity of each gate:} That is the variables of each tensor are bipartite into sets $\variablesetof{\insymbol}$ and $\variablesetof{\outsymbol}$ of same cardinality and the tensor is unitary.
%    \begin{align*}
%        \unitaryat{\ccatvariable{\insymbol},\ccatvariable{\outsymbol}}
%        \defcols \bigotimes_{\atomenumerator\in\variablesetof{\insymbol}} \mathbb{C}^2 \rightarrow \bigotimes_{\atomenumerator\in\variablesetof{\outsymbol}} \mathbb{C}^2  \, ,
%    \end{align*}
%    is a unitary map, that is
%    \begin{align*}
%        \left(T_{\insymbol \rightarrow \outsymbol}\right)^H \circ \left(T_{\insymbol \rightarrow \outsymbol}\right)
%        = \contractionof{
%            T_{\insymbol \rightarrow \outsymbol}[\ccatvariable{\insymbol},\seccatvariable],
%            \overline{T}_{\insymbol \rightarrow \outsymbol}[\seccatvariable,\ccatvariable{\outsymbol}]
%        }{\ccatvariable{\outsymbol},\ccatvariable{\insymbol}}
%        = \identityat{\ccatvariable{\outsymbol},\ccatvariable{\insymbol}}.
%    \end{align*}
...
\begin{align*}
    ...
    }{\cavariable{\insymbol},\cavariable{\outsymbol}} \, .
\end{align*}
...
\subsubsection{Computational basis measurement}
...
\begin{figure}
    \begin{center}
        \input{tikz/circuit_notation/directed_measurement_sketch.tikz}
    \end{center}
...
\begin{align*}
    \qstatewith
    = \contractionof{\expof{i\cdot\phasecorewith}, \absof{\qstatewith}}{\shortcatvariables}\, .
\end{align*}

The measurement distribution depends only on $\absof{\phi}$, that is
\begin{align*}
    \probwith
    = \absof{\qstatewith}^2 \, .
\end{align*}
...
\begin{figure}
    \begin{center}
        \input{tikz/circuit_notation/phase_contraction_measurement.tikz}
    \end{center}
...
The most general measurement formalism is through a POVM, a set
\begin{align*}
    \{E_\seccatindex[\ccatvariable{\insymbol},\ccatvariable{\outsymbol}] \wcols \seccatindex\in[r]\}
\end{align*}
of positive operators with
\begin{align*}
    \sum_{\seccatindex\in[r]} E_\seccatindex[\ccatvariable{\insymbol},\ccatvariable{\outsymbol}] 
    = \identityat{\ccatvariable{\insymbol},\ccatvariable{\outsymbol}}
\end{align*}

Measuring a pure state $\ket{\psi}$ we get the outcome $\seccatindex$ with probability
\begin{align*}
    \contraction{\qstateat{\ccatvariable{\outsymbol}},E_\seccatindex[\ccatvariable{\insymbol},\ccatvariable{\outsymbol}],\qstateat{\ccatvariable{\insymbol}}}
\end{align*}

Introducing $\headvariable$ as an enumeration we define the tensor
\begin{align*}
    E[\headvariable,\ccatvariable{\insymbol},\ccatvariable{\outsymbol}]
    = \sum_{\seccatindex\in[r]} \onehotmapofat{\seccatindex}{\headvariable} \otimes E_\seccatindex[\ccatvariable{\insymbol},\ccatvariable{\outsymbol}] \, .
\end{align*}

The condition on $E$ is then
\begin{align*}
    \contractionof{E[\headvariable,\ccatvariable{\insymbol},\ccatvariable{\outsymbol}]}{\ccatvariable{\insymbol},\ccatvariable{\outsymbol}}
    = \identityat{\ccatvariable{\insymbol},\ccatvariable{\outsymbol}} \, .
\end{align*}

The measurement distribution of a pure state is then
\begin{align*}
    \contractionof{\qstateat{\ccatvariable{\outsymbol}},E[\headvariable,\ccatvariable{\insymbol},\ccatvariable{\outsymbol}],\qstateat{\ccatvariable{\insymbol}}}{\headvariable} \, .
\end{align*}
When having a mixed state defined by a density matrix $\rho[\ccatvariable{\insymbol},\ccatvariable{\outsymbol}]$, the distribution is
\begin{align*}
    \contractionof{E[\headvariable,\ccatvariable{\insymbol},\ccatvariable{\outsymbol}],\rho[\ccatvariable{\insymbol},\ccatvariable{\outsymbol}]}{\headvariable} \, .
\end{align*}



In our diagramatic picture, POVMs are tensors
\stantikzpic{
    \drawtensorblock{0}{0}{2}{$E$}
    \drawhwire{-4}{0}{$\ccatvariable{\insymbol}$}
    \drawhwire{2}{0}{$\ccatvariable{\outsymbol}$}
    \drawvwire{0}{-1}{$\headvariable$}
}
such that
\stantikzpic{
    \drawtensorblock{0}{0}{2}{$E$}
    ...
    \drawhwire{8}{0}{$\ccatvariable{\insymbol}$}
    \drawhwire{10}{0}{$\ccatvariable{\outsymbol}$}
    \drawvariabledot{10}{0}
}

%%
The standard computational basis measurement is
\stantikzpic{
%    \drawtensorblock{0}{0}{2}{$E^{cb}$}
%    \drawhwire{-4}{0}{$\ccatvariable{\insymbol}$}
%    \drawhwire{2}{0}{$\ccatvariable{\outsymbol}$}
%    \drawvwire{0}{-1}{$\headvariable$}
%%    \drawvariabledot{0}{-3}
...
%    \drawtextnode{6}{0}{$=$}
    ...
    \drawhwire{10}{0}{$\ccatvariable{\outsymbol}$}
    \drawvariabledot{10}{0}
    \drawvwire{10}{0}{$\headvariable$}
}
and satisfies these criteria.
...
%with slices
%\begin{align*}
%    E[\headvariable=\seccatindex,\ccatvariable{\insymbol},\ccatvariable{\outsymbol}] = E_\seccatindex \, .
%\end{align*}
...
%    \contractionof{\tnetof{\graph}[\ccatvariable{\insymbol}],E[\headvariable,\ccatvariable{\insymbol},\ccatvariable{\outsymbol}],\tnetof{\secgraph}[\ccatvariable{\outsymbol}]}{\headvariable} \, .
%\end{align*}
