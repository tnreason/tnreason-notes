\begin{example}[Minterm formulas build simplices]
    ...
    \begin{align*}
        ...
        = \bigwedge_{\selindex\in[\catorder]} \lnot^{1-\imelementat{\indexedselvariable}} \, \catvariableof{\selindex}
...
%    We store them by the tuple $\hardparam$, where $\hardlegset$ is the support of $\secbasemeasure$ and $\headindexof{\selindex}=0$ for $\selindex\in\hardlegset$.
    ...
    % 
    More generally, the mean polytope to a Boolean statistic is a standard simplex, if and only if each target coordinate restriction $\formulaof{\secselindex}$ contradicts all others, that is
    \begin{align*}
