\section{Implementation in the \python{} library \tnreason{}}\label{sec:implementation}
...
The concepts presented in this paper have been implemented in the open source \python{} library \tnreason{}\footnote{\tnreason{} is available in version 2.0.0 at \url{pypi.org/tnreason} and maintained at \url{github.com/tnreason/tnreason-py}.}.
In this section, we explain the basic design and functionality of this library and draw close connections to the theoretical exposition in the previous sections. In particular, Appendix~\ref{sec:algExaImplementation} provides detailed implementations of the algorithms and examples in this work.
...
In the subpackage \tnreason{}.\spreasoning{} we implement reasoning algorithms, such as generalizations of the message passing algorithms presented in \algoref{alg:treeBeliefPropagation}, \algoref{alg:constraintPropagation}, and \algoref{alg:AMM_HLN}.
In the subpackage \tnreason{}.\spapplication{} one can construct tensor network encodings of propositional formulas and datasets.
...
The \mintinline{python}{posDict} are \mintinline{python}{dict} instances, where the keys are the \mintinline{python}{str} tensor colors and the values are \mintinline{python}{int}.
Each \mintinline{python}{posDict} collects leg vectors of the corresponding elementary tensor that are not trivial. These leg vectors are the basis vectors enumerated by the corresponding \mintinline{python}{int} value.

Single tensor coordinates can be retrieved by indexing with a \mintinline{python}{posDict}.
We can, for example, check whether \mintinline{python}{{"X_0":0,"X_1":1,"X_2":0}} is a model:
\begin{minted}{python}
...
\end{minted}
Here we apply a nested-list description of decomposition hypergraphs (see \defref{def:decompositionHypergraph}) with a specification of the logical connectives in the first position of the list (by \mintinline{python}{"and","or","not"} we refer to the connectives $\land,\lor,\lnot$).
...
\end{minted}
A depiction of the underlying hypergraph as a factor graph, which highlights edges as blue blocks and nodes as red blocks, can be created with \mintinline{python}{engine.draw_factor_graph(fDecomp)} (see \figref{fig:factorGraphPlt}).
...
Single tensors can be obtained by contracting a tensor network while specifying the open variables (for an explanation of the suffixes, see \figref{fig:factorGraphPlt}), for example:
\begin{minted}{python}
...
\begin{figure}
    ...
    \end{center}
    \caption{
        Factor graph highlighting a tensor network decomposition of the syntactic decomposition of the propositional formula of \exaref{exa:propFormulaHeadSym}.
        Blue blocks highlight hyperedges carrying tensors and red blocks highlight variables.
