...
%{\alex{Joint distributions of two Booleans, sufficient statistic by head count (coin toss interpretation) and exponential family in case of independence}}
    In general, joint distribution of two Boolean variables $\catvariableof{0},\catvariableof{1}$ are $2\times 2$ matrices of non-negative coordinates summing to 1:
    \begin{align*}
        \probat{\catvariableof{[2]}}
        = \begin{bmatrix}
              p_{0,0} & p_{0,1} \\
              p_{1,0} & p_{1,1}
        \end{bmatrix}
    ...
    \begin{align*}
        ...
        \end{bmatrix}
    \end{align*}
    This symmetry also implies, that the distributions are identically distributed, that is for any $\catindex\in\{0,1\}$ we have % with $\mathbb{I}_2 = (1,1)^\intercal$:
    \begin{align*}
...
%    \end{align*}
    ...
    \begin{align*}
        ...
        =\frac{1}{(1+\expof{\canparam})^2} \begin{bmatrix}
                                               1 \\
                                               \expof{\canparam}
        \end{bmatrix}
        \begin{bmatrix}
