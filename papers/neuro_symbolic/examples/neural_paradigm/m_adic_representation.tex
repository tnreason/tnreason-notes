\begin{example}[Sum of integers in $\catdim$-adic representation]
    ...
    \end{align*}
    Note that the tensor space of $\bencodingof{+}$ is of dimension $\catdim^{3\cdot\catorder+1}$ increasing exponentially in $\catorder$.
...
%    Let $X^i_{[d]}$ be the $d$ bits of the $i$th number, and let $X^{[n]}_{[d]}$ be all the bit variables (that is $n\cdot d$ many) of the $n$ numbers.
