...
The mean polytope is then a 0/1-polytope \cite{ziegler_lectures_2000}, since its vertices are exactly the contained boolean vectors.
...
\begin{definition}
    \label{def:cubeLike}
    We say that a face $\facesymbol$ of $\genmeanset$ is cube-like, if it is empty or there is $\variableset\subset[\seldim]$ and $\headindexof{\variableset}\in\bigtimes_{\selindex\in\variableset}[2]$ such that
    \begin{align*}
        \facesymbol 
        = \genmeanset \cap \facesymbolof{(\variableset,\headindexof{\variableset})} \, .
    \end{align*}
    Here we denote by $\facesymbolof{(\variableset,\headindexof{\variableset})}$ the faces of the hypercube $\cubeof{\seldim}$ (see \exaref{exa:hypercubeFaces}).
...
\begin{proof}
    ...
    (i)$\Rightarrow$(ii):
    Let us assume that $\facesymbol$ is cube-like, that is there is $\variableset\subset[\seldim]$ and $\headindexof{\variableset}\in\bigtimes_{\selindex\in\variableset}[2]$ such that $\facesymbol = \genmeanset \cap \facesymbolof{(\variableset,\headindexof{\variableset})}$.
    We use that $\facesymbol,\genmeanset$ and $\facesymbolof{(\variableset,\headindexof{\variableset})}$ are the convex hulls of the cube vertex sets $\imsetof{\hlnstat,\meanset}{\facesymbol}$, $\imsetof{\hlnstat,\meanset}{}$ and
    ...
    \begin{align*}
        \sum_{\imelementwith\in\imsetof{}{(\variableset,\headindexof{\variableset})}}\onehotmapofat{\imelementwith}{\headvariables}
    ...
    \begin{align*}
        \facesymbol = \facesymbolof{(\variableset,\headindexof{\variableset})} \cap \genmeanset \, .
    \end{align*}
    We conclude that $\facesymbol$ is cube-like.
\end{proof}

%% Reference to simplices and hypercubes
\theref{the:faceMeasureHardLogicNetworks} implies that hypercubes (see \exaref{exa:hypercubeFaces}) and simplices (see \exaref{exa:simplexFaces}) are cube-like polytopes, since their faces are intersections with faces of the hypercube.


%Let us now give with the standard simplices a class of convex polytopes, which are not hypercubes, but cube-like.
%\input{examples/mean-polytope/simplex_faces}
...
\input{examples/boolean-stat/atomic_formulas}

\input{examples/boolean-stat/minterm_formulas}
