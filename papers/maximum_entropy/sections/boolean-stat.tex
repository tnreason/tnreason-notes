...
We now constructively show, that any convex polytope with boolean vertices in $\parspace$ (a so called 0-1 polytope, see \cite{ziegler_lectures_2000}) is the mean polytope to a boolean statistic.
...
\begin{theorem}
    ...
    \begin{align*}
        ...
        \bigwedge_{\imelementwith \in \restrictionofto{\meanset}{\rr^\selindex\times 0_{\seldim-\selindex}}\cap \{0,1\}^{\seldim}}
        \left(\left(\bigwedge_{\secselindex\in[\selindex+1]} \lnot^{1-\imelementat{\selvariable=\secselindex}} \formulaofat{\secselindex}{\catvariableof{[\secselindex]}}\right) \Rightarrow
            \begin{cases}
...
\begin{proof}
    ...
    \begin{align}
        ...
        = \restrictionofto{\meanset}{\rr^\selindex\times 0_{\seldim-\selindex}} \, .
    \end{align}
    ...
    Then for any $\shortcatindicesin$ there is exactly one $\imelementwith \in \restrictionofto{\meanset}{\rr^\selindex\times 0_{\seldim-\selindex}}$ such that $\shortcatindices$ is a model of
    \begin{align*}
        \formulaofat{\selindex,v}{\catvariableof{[\selindex+1]}}\coloneqq
        \bigwedge_{\secselindex\in[\selindex+1]} \lnot^{1-\imelementat{\selvariable=\secselindex}} \formulaofat{\secselindex}{\catvariableof{[\secselindex]}} \, .
    \end{align*}
