...
\end{align*}
containing all mean parameters to the distributions in $\distmeasuredby{\basemeasure}$.
By $\sencsstatwith$ we denote the selection encoding of the statistic (see introduction).
%% Convex hull
Since $\distmeasuredby{\basemeasure}$ is the convex hull of the vertices $\frac{1}{\basemeasureat{\indexedshortcatvariables}} \cdot \onehotmapofat{\shortcatindices}{\shortcatvariables}$, and any mean parameters are linear transforms of the distributions, the mean polytope is the convex hull of the vectors
\begin{align*}
    \genimset
...
We here define them based on convex hulls and introduce tensor representations by refined base measures.
...
The set $\facelatticeof{\genmeanset}$, partially ordered by the inclusion operation, is a lattice \cite{ziegler_lectures_2013}.
% Refined base measures
We now show that each face itself is a convex polytope to a refined base measure.
...
The refined base measures has by definition a smaller support than the base measure.
...
While vertices are the minimal non-vanishing faces in the face-lattice (see \cite{ziegler_lectures_2013}), we now show that also the maximal face, namely the polytope itself, is representable with respect to the elementary hypergraph $\elgraph$ (see \figref{fig:elFormat}).

\input{examples/mean-polytope/maximal-face}

\begin{figure}
    \input{tikz/mean-polytope/el-format}
    \caption{Elementary tensor format consiting in the elementary hypergraph $\elformat$ $a)$, and corresponding tensor networks $b)$ being the tensor products of vectors.}
    \label{fig:elFormat}
\end{figure}
...
\begin{figure}
    ...
    \caption{The $\cpformat$ tensor format consist in the $\cpformat$ hypergraph a) and decorating tensor networks b).
