...
\begin{proof}
    ...
    Therefore, if $\secprobtensor$ does not coincide with $\expdistof{(\sstat,\canparam,\basemeasure)}$, it is not a maximum entropy distribution.
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    By \lemref{lem:expFamilyMaxEnt} we have that $\expdistof{(\sstat,\canparam,\basemeasure)}$ is the maximum entropy distribution with respect to the moment constraint on $\sstat$ by $\meanparamwith$ and the base measure $\basemeasure$.
\end{proof}
