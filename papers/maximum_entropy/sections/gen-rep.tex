...
\begin{theorem}[Generic characterization of Maximum Entropy Solutions]
    ...
    If $\meanparamwith\in\genmeanset$ the maximum entropy distribution is $\expdistof{\sstat,\weightparam,\genfacemeasure}$ where $\facesymbol=\faceto{\meanparam}$ (see \lemref{lem:faceToMean}) and
    \begin{align*}
        \weightparam
    ...
    For any hypergraph $\graph$ and any face $\facein$ we have that all maximum entropy distributions to mean parameters on $\interiorof{\facesymbol}$ are in $\realizabledistsof{\sstat,\graph,\basemeasure}$ if $\facesymbol$ is representable with respect to $\graph$ (see \defref{def:faceRepresentability}).
\end{theorem}
