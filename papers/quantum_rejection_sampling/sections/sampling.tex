...
\begin{itemize}
    \item Find a circuit preparing the q-sample of an ancilla augmentation
...
%We can produce Q-samples for ancilla augmented \ComputationActivationNetworks{}  using \computationCircuits{} and \activationCircuits{} (see \figref{fig:caCircuit}):
%\begin{itemize}
%    \item For each (sub-) statistic, prepare a qubit by \ComputationCircuits{}.
%    We do not have to uncompute the statistic qubits.
%    \item Based on the computed qubits, prepare ancilla qubits by \ActivationCircuits{} to the activation cores.
%    This prepares the q-sample of an ancilla augmented \ComputationActivationNetwork{}.
%\end{itemize}
