\section{Quantum Rejection Sampling from \ComputationActivationNetworks{}}\label{sec:sampling}

\red{Works in general for augmented distributions, which can be prepared as measurement distributions.
We do not use that the distributed variables are uniform in the augmented distribution, nor that the prepared state is a q-sample (i.e. has vanishing phase core).}

\subsection{Amplitude amplification for augmented distributions}

When $\qstateofat{0}{\avariables,\headvariables,\shortcatvariables}$ has as measurement distribution the augmentation $\secprobat{\avariables,\headvariables,\shortcatvariables}$ of $\probat{\headvariables,\shortcatvariables}$, then there are states $\qstateofat{\goodstatesymbol}{\avariables,\headvariables,\shortcatvariables}$ and $\qstateofat{\badstatesymbol}{\avariables,\headvariables,\shortcatvariables}$ with
\begin{align*}
    \qstateofat{0}{\avariables,\headvariables,\shortcatvariables}
    = \sqrt{\acceptanceprob} \cdot \qstateofat{\goodstatesymbol}{\avariables,\headvariables,\shortcatvariables}
    + \sqrt{1-\acceptanceprob} \cdot \qstateofat{\badstatesymbol}{\avariables,\headvariables,\shortcatvariables}
\end{align*}
and 
\begin{align*}
    \qstateofat{\badstatesymbol}{\avariables=\onetuple{[\seldim]},\headvariables,\shortcatvariables}
    = \zerosat{\headvariables,\shortcatvariables}
\end{align*}
and for $\aindexof{[\seldim]}\neq\onetuple{[\seldim]}$
\begin{align*}
    \qstateofat{\goodstatesymbol}{\avariables=\aindexof{[\seldim]},\headvariables,\shortcatvariables}
    = \zerosat{\headvariables,\shortcatvariables} \, .
\end{align*}
That is, the measurement distributions are disjoint, since to $\goodstatesymbol$ supported only for $\aindexof{[\seldim]}=\onetuple{[\seldim]}$ and for $\badstatesymbol$ not supported for $\aindexof{[\seldim]}=\onetuple{[\seldim]}$.
...
Now $\repnum\in\nn$ consecutive reflections of $\qstateof{\goodstatesymbol}$ and $\qstateof{0}$ result in the state (see \figref{fig:rotationSketch})
\begin{align*}
...
To amplify we want $\left(\frac{1}{2}+\repnum\right)\rotanglesymbol$ to be as close to $\frac{\pi}{2}$ and avoid overrotation and therefore choose
\begin{align*}
    \repnumof{\optsymbol}
    = \left\lfloor \frac{\pi}{2 \rotanglesymbol} -\frac{1}{2}  \right\rfloor
    = \left\lfloor \frac{\pi}{4 \arcsin\left(\sqrt{\acceptanceprob}\right)} -\frac{1}{2}  \right\rfloor  \, .
\end{align*}

We further have the bound
\begin{align*}
   \repnumof{\optsymbol}
   \leq \left\lfloor \frac{\pi}{4 \sqrt{\acceptanceprob}} -\frac{1}{2}  \right\rfloor
\end{align*}
which is accurate for small $\acceptanceprob$.

% Comparison with classical rejection sampling
...
\input{examples/sampling/formula-sampling}

\input{examples/sampling/toy-accounting-sampling}
...
%\begin{itemize}
%    \item Represent the distribution to be sampled as a conditionalization of a preparable distribution
...
%\begin{align*}
%    \qstateofat{0}{\avariables,\headvariables,\shortcatvariables}
%    = \sinof{\frac{\rotanglesymbol}{2}} \qstateofat{\goodstatesymbol}{\avariables,\headvariables,\shortcatvariables}
%    + \cosof{\frac{\rotanglesymbol}{2}} \qstateofat{\badstatesymbol}{\avariables,\headvariables,\shortcatvariables}
%\end{align*}
...
%    \end{align*}
%    \item Reflection $\reflectionof{\qstate}$ along $\qstate$, by
%    \begin{align*}
...
%we get a bound
%\begin{align*}
%    \repnumof{\optsymbol} \leq \frac{\pi}{4\sqrt{\acceptanceprob}} \, .
%\end{align*}
