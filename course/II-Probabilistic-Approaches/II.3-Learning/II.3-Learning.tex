...
%	\[ \datacoreof{\atomenumerator}_{: \datindex} =\onehotmapof{\catindexof{\atomenumerator}^\datindex} \]
    ...
    \begin{frame}{Log-likelihood}
        Likelihood of a dataset drawing data indepdently factorizes:
        \begin{align*}
            \prod_{\datindexin} \probat{\shortcatindices^{\datindex}}
        \end{align*}

        Taking the log and averaging:

        This is the \emph{cross-entropy} between the empirical distribution and the candidate distribution $\probwith$.
