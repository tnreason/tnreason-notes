...
Here, they are provided in our tensor networks notation and for hypergraphs for completeness. % Since typically HC is shown on graphs.
...
Different to the original statement (see \cite{clifford_markov_1971}), we proof the analogous statement for hypergraphs, where we have to demand the property of clique-capturing defined in \defref{def:ccHypergraph}.
...
\begin{proof}
    ...
    \end{align*}
    Applying this in each squares bracket term of \eqref{eq:indContFacProof}, we get that
    \begin{align*}
        \frac{
            Z_{\thirdnodes}
        }{
