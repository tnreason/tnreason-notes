...
Let us now provide proofs for the factorization theorems stated in \secref{sec:prob_rep}.
...
We here provide them in our tensor networks notation and for hypergraphs for completeness. % Since typically HC is shown on graphs.
...
\begin{proof}[Proof of \theref{the:factorizationHammersleyClifford}]
    ...
    \begin{align*}
        ...
        \right)^{(-1)^{\cardof{\secnodes}-\cardof{\thirdnodes}}} \, .
    \end{align*}
    Using the assumption of \theref{the:factorizationHammersleyClifford} we find for any subset $\secnodes\subset\nodes$, which is not contained in a hyperedge, $\nodea,\nodeb \in\secnodes$ such that $\catvariableof{\nodea}$ is independendent on $\catvariableof{\nodeb}$ conditioned on $\catvariableof{\secnodes/\{\nodea,\nodeb\}}$.
    ...
    $i)\Rightarrow ii)$:
    Let us now show the converse statement and assume that there is a Markov Network representing the distribution $\probwithnodes$, and let us choose subsets $\nodesa,\nodesb,\nodesc\subset\nodes$ such that $\nodesc$ separates $\nodesa$ from $\nodesb$.
...
Since sufficient statistics are sometimes introduced based on the data processing inequality (see e.g. \cite{cover_elements_2006}), we also show that also that definition is equivalent to the factorization of the family.
...
%            I(Z;X) = I(\thirdcatvariable;\headvariableof{\sstat}) \, .
        \end{align*}
        \item[ii)] $\thirdcatvariable\rightarrow\headvariableof{\sstat}\rightarrow\catvariable$ is a Markov Chain, i.e.
        \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    Using the KL-divergence characterization of the mutual information, this is equal to % Details needed?
    \begin{align*}
    ...
    There is thus a tensor $\acttensor$ such that for all $\thirdcatindex\in\valof{\thirdcatvariable}$ and $\catindex\in\valof{\catvariable}$
    \begin{align*}
...
%    \end{align*}
    ...
    \begin{align*}
        ...
        &= \condprobat{\indexedthirdcatvariable}{\headvariableof{\sstat}=\sstatat{\catindex}} \, .
    \end{align*}
    We further have at almost all $\headindexof{\sstat}\in\valof{\headvariableof{\sstat}}$, $\thirdcatindex\in\valof{\thirdcatvariable}$ and $\catindex\in\valof{\catvariable}$ that $\headindexof{\sstat}=\sstatat{\catindex}$ and
    \begin{align*}
