...
\subsection{Algorithm~\ref{alg:treeBeliefPropagation}, \ref{alg:directedBeliefPropagation} and \ref{alg:constraintPropagation} (Tree, Directed Belief and Constraint Propagation)}

The three message passing algorithms are implemented as functions as one class \mintinline{python}{ContractionPropagation}, since they share common structure.
...
\subsection{Example~\ref{exa:madicRepresentation} and \ref{exa:madicPropagation} (Integer Summation in $\catdim$-adic Representation)}

Following the decomposition of summations in $\catdim$-adic into local summations, the function \mintinline{python}{get_sum_tn} produces a corresponding tensor network of basis encodings.
