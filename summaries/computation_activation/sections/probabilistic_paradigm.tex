...
\janina{The following theorem states, that the contraction of a whole tensor network can be replaced by local contractions with derived messages.
...
We denote for each pair $(\sedge,\redge)$ the subset $\preedgeset\subset\edges$ as the subset of edges $\edgein$, for which each path to $\redge$ passes through $\sedge$. \janina{The tree hypergraph property makes this definition equivalent to an existing path through $\sedge$, which is used in the proof of the following lemma.}
