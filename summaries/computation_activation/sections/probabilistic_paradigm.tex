...
\begin{algorithm}[hbt!]
    \caption{Tree Belief Propagation}\label{alg:beliefPropagation}
    \begin{algorithmic}
        \Require Tensor network $\extnet$ on a hypergraph $\graph$
        \Ensure Messages $\{\messagewith\wcols(\secsedge,\sedge)\in\dirovedges\}$
        \iosepline
...
\begin{theorem}\label{the:treeBeliefPropagationExactness}
    \alex{The final messages in the tree belief propagation are contracted to local marginals.}
\end{theorem}
