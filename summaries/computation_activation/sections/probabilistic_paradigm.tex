...
Furthermore, we present the \computationMechanism{}, in which the Fisher-Neyman Factorization Theorem is used to decompose distributions in the presence of sufficient statistics.
...
\begin{definition}
    ...
    For any distribution $\probat{\catvariableof{0},\catvariableof{1}}$ the marginal distribution is given by the contraction (see \defref{def:contraction})
    \begin{align*}
...
\subsection{The independence mechanism: Factorization into graphical models}
...
\begin{definition}[Conditional independence]
    ...
    Assume a joint distribution of variables $\catvariableof{0}$, $\catvariableof{1}$ and $\catvariableof{2}$ such that $\margprobat{\catvariableof{2}}$ is positive.
...
\subsection{The computation mechanism: Factorization based on sufficient statistics}
...
\begin{corollary}
    ...
    Then, there is a tensor $\basemeasurewith$ and an activation tensor $\acttensorat{\headvariables,\thirdcatvariable}$ such that for any $\thirdcatindex\in\mathrm{val}(\thirdcatvariable)$ we have
    \begin{align*}
...
The resulting algorithm is called Belief Propagation (see \algoref{alg:treeBeliefPropagation}).
...
For our purposes we call a hypergraph $\graph$ a tree when the graph with nodes by the hyperedges $\dirovedges$ and edges by $\dirovedges$ is a tree (for an example see \figref{fig:studentMessagePassingDirections}).
...
\begin{algorithm}[hbt!]
    ...
    \begin{algorithmic}
        ...
        \iosepline
        \State Initialize $\scheduler=\{(\secsedge,\sedge) \wcols \secsedge \text{ a leaf in } (\edges,\dirovedges)\}]$
...
We denote for each pair $(\sedge,\redge)$ the subset $\preedgeset\subset\edges$ as the subset of edges $\edgein$, for which each path to $\redge$ passes through $\sedge$. The tree hypergraph property makes this definition equivalent to an existing path through $\sedge$, which is used in the proof of the following lemma.
