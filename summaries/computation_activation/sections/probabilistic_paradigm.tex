...
\begin{corollary}
    ...
    Then there is a tensor $\basemeasurewith$ and an activation tensors $\acttensorat{\headvariables,\thirdcatvariable}$ such that for any $\thirdcatindex\in\mathrm{val}(\thirdcatvariable)$
    \begin{align*}
...
Let us now show in two examples, how families of distributions can be represented in \CompActNets{} by sufficient statistics.
...
Let us now show, that exponential families are specific instances of \CompActNets{}, whose activation tensors have elementary decompositions.
% Universal properties
The importance of exponential families in statistics stems from their universal properties.
A classical theorem by Pitman, Koopman and Darmois (see \cite{pitman_sufficient_1936}) states, that whenever a family with constant support and a finite sufficient statistic for arbitrary large data sets is in an exponential family.
...
\begin{definition}[Exponential Family]\label{def:expFamily}
    ...
    \end{align*}
    We then call the tensor $\probfamilyofat{\sstat,\basemeasure}{\shortcatvariables}{\Theta}$ with $\valof{\Theta}=\parspace$ and slices for $\canparam\in\parspace$ by
    \begin{align*}
...
To see that \defref{def:expFamily} is consistent with the typical definition of exponential families (see \cite{brown_fundamentals_1987}), let us notice that for each $\canparam\in\parspace$ the normalization amounts to the division by
\begin{align*}
...
Then we have for each coordinate $\shortcatindicesin$
\begin{align*}
