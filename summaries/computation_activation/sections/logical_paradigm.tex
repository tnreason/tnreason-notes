...
\section{The Logical Paradigm}\label{sec:log_rep}
...
\alex{Propositional formulas of concern often have a syntactical specification as composed functions.
...
\end{theorem}
\begin{proof}
    Since $\seckb\models\formula$ it holds that $\contraction{\seckb[\shortcatvariables],\lnot\formula[\shortcatvariables]}=0$ and thus  $\contractionof{\seckb[\shortcatvariables],\lnot\formula[\shortcatvariables]}{\shortcatvariables}=\zerosat{\shortcatvariables}$.
...
\begin{proof}
    ...
    \begin{align*}
       ...
       %&= \contractionof{\kbwith,\formulawith}{\shortcatvariables} \, .
    \end{align*}
    Since $\contractionof{\kbwith,\lnot\formulawith}{\shortcatvariables}$ is boolean, we thus have that $\kbwith=\contractionof{\kbwith,\formulawith}{\shortcatvariables}$ if and only if $\contraction{\kbwith,\lnot\formulawith}=0$, that is $\kb\models\formula$.
\end{proof}
...
This motivates a message-passing approach to decide entailment by iteratively adding entailed formulas to the knowledge base and checking entailment on smaller parts of the knowledge base.
...
\begin{theorem}
    ...
    \alex{All messages during constraint propagation are sound, \textbf{i.e}
    \begin{align*}
...
\end{theorem}
\begin{proof}
    By induction using the monotonocity \theref{the:monotonicityPL} and invariance under adding entailed formulas \theref{the:addingEntailed}.
\end{proof}
