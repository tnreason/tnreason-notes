...
Let us now exploit the common formulation of logical formulas and probabilistic models in \CompActNets{} to define hybrid models, which combine both aspects.
We call \CompActNets{} in the special case of boolean statistics $\sstat$ \HybridLogicNetworks{}.

\subsection{Parametrization}

We now investigate hybrid logical and probabilistic models, which we want to call \HybridLogicNetworks{} (HLNs).
...
\begin{align*}
    ...
    \begin{cases}
        ...
        \lnof{\frac{\acttensorlegat{\headvariableof{\selindex}=1}}{\acttensorlegat{\headvariableof{\selindex}=1}}} & \ifspace \selindex\notin\hardlegset \, .
    \end{cases}
\end{align*}
...
\input{../examples/hln_accounting_rep}

\subsection{Parameter Estimation in \HybridLogicNetworks{}}

%% Likelihood maximation
Let us now briefly discuss how \HybridLogicNetworks{} can be trained on data based on likelihood maximization.
Given a dataset $\dataset$ consisting of $\datanum$ independent and identically distributed samples from an unknown distribution, we want to find a \HybridLogicNetwork{} $\hlnwith$ that maximizes the data likelihood
\begin{align*}
    \lossof{\hybridparam} \coloneqq -\frac{1}{\datanum} \sum_{\decindexin} \lnof{\probofat{\hlnparameters}{\shortcatvariables=\shortcatindices^{\datindex}}}
    \, .
\end{align*}
We notice that this is $\infty$ if and only if there is a data point $\decindexin$ with
\begin{align*}
    \hlnformulaat{\shortcatvariables=\shortcatindices^{\datindex}}=0 \, .
\end{align*}
If this is not the case, we can rewrite the loss using the empirical mean vector $\datameanwith\in\parspace$, which is defined for $\selindexin$ as
\begin{align*}
    \datameanat{\indexedselvariable}
    = \frac{1}{\datanum} \sum_{\decindexin} \formulaofat{\selindex}{\shortcatvariables=\shortcatindices^{\datindex}} \, ,
\end{align*}
by
\begin{align*}
    \lossof{\hybridparam} =
...
\begin{align*}
    \hardlegset = \{\selindex \wcols \datameanat{\indexedselvariable}\in\ozset\} \andspace \headindexof{\selindex} = \datameanat{\indexedselvariable} \, .
\end{align*}
We further optimize the coordinates $\selindex\in[\seldim]/\hardlegset$ of $\canparamwithin$ alternatingly by the coordinate descent steps
\begin{align*}
...
\end{align*}
Based on an interpretation of the coordinate descent steps as matching steps for the mean parameters or moments to $\formulaof{\selindex}$, we call this method alternating moment matching for \HybridLogicNetworks{} and provide pseudocode for it it in \algoref{alg:AMM_HLN}.
%% Forward inference in inner loop
We notice, that during the coordinate descent steps the computation of the marginal probability of the variable $\headvariableof{\selindex}$ with respect to the current network parameters is required.
...
\begin{algorithm}[hbt!]
    ...
    \begin{algorithmic}
        ...
        \While{Convergence criterion is not met}
            \ForAll{$\selindex\in[\seldim]/\hardlegset$}
                \State Compute
                \begin{align*}
                    \hypercoreat{\headvariableof{\selindex}}
                    = \contractionof{\{\bencodingof{\enumformula} \wcols \selindexin\}
...
\input{../examples/hln_accounting_amm}

\subsection{Entailment by \HybridLogicNetworks{}}
...
\begin{theorem}%[{~\cite[Theorem 8.12]{goessmann_tensor-network_2025}}]
    \alex{Unclear: Would need to add probabilistic entailment..}
    Let $\probofat{\hlnparameters}{\shortcatvariables}$ be a \HybridLogicNetwork{}.
    Given a query formula $g$, we have that $\probofat{\hlnparameters}{\shortcatvariables}\models~g$ if and only if
    \begin{align*}
        \hlnformula \models g,
    \end{align*}
...
\end{theorem}
\begin{proof}
    \alex{Could be done based on support.}
\end{proof}
