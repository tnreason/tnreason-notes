...
\input{../examples/soft_activation/coin_toss_family}
...
\input{../examples/soft_activation/coin_toss_hc.tex}

\input{../examples/soft_activation/student_example_hc}

Equivalently, \theref{the:factorizationHammersleyClifford} states that for any strictly positive joint distribution $\probwithnodes$ whose conditional independencies are exactly those encoded by a clique–capturing hypergraph $G=(V,E)$, there exist non–negative activation cores $\tau_e[X_e]$ such that
\begin{align*}
    \probwithnodes
    = \normalizationof{\extnetasset}{\nodevariables} \, .
\end{align*}
Thus, the conditional–independence structure of $\probwithnodes$ determines a global tensor–network decomposition of $\probwithnodes$.
...
\alex{Add the Student example in Koller book, demonstration of clique-capturing and Hammersley Clifford.}
...
\input{../examples/soft_activation/sufstat_probability}
...
\input{../examples/soft_activation/coin_toss_ft}
...
\input{../examples/soft_activation/graphical_models_as_cans}
...
\input{../examples/soft_activation/two_boolean_cascade}


\subsection{Efficient Contractions by Message Passing}

We now formulate the Belief Propagation algorithm based on tensor network.
...
\begin{algorithm}[hbt!]
    \caption{Belief Propagation}\label{alg:beliefPropagation}
    \begin{algorithmic}
