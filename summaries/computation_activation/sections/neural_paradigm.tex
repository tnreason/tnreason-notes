...
\end{theorem}
\begin{proof}
    We show by an induction over the number of edges in $\graph$, that for any $\shortcatindicesin$ and $\node\in\nodes$ we have that
    \begin{align}
    ...
    \begin{align*}
        ...
        \onehotmapofat{\exfunctionof{\node}(\shortcatindices)}{\catvariableof{\node}} \, .
    \end{align*}
    Here we used in the second equation the induction hypothesis on the subgraph $(\nodes,\edges/\{\secedge\})$ with $\cardof{\edges/\{\secedges\}}=n$ and in the third equaltion a generic contraction property of basis encodings.
