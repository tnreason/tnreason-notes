...
\begin{proof}
    ...
    \begin{align*}
        ...
        \contractionof{
            ...
            \contractionof{\left\{\bencodingofat{\secexfunctionof{\edge}}{\catvariableof{\edge}} \wcols \edge\in\edges/\{\secedge\}\right\}
        ...
        \onehotmapofat{\exfunctionof{\node}(\shortcatindices)}{\catvariableof{\node}} \, .
    \end{align*}
    Here we used, in the second equation, the induction hypothesis on the subgraph $(\nodes,\edges/\{\secedge\})$ with $\cardof{\edges/\{\secedge\}}=n$ and, in the third equation, a generic contraction property of basis encodings.

    Thus, \eqref{eq:indHypNeurThm} holds always and we thus have for any $\shortcatindicesin$
    \begin{align*}
        \contractionof{\left\{\bencodingofat{\secexfunctionof{\edge}}{\catvariableof{\outgoingnodes},\catvariableof{\incomingnodes}} \wcols \edge=(\incomingnodes,\outgoingnodes)\in\edges\right\}
...
\begin{algorithm}[hbt!]
    ...
    \begin{algorithmic}
        ...
        \Ensure Messages $\{\messagewith\wcols(\secsedge,\sedge)\in\dirovedges\}$ \janina{is e0, e1 fixed? then i would change the notation in the loop. if not, i would write it in the set.}
        \iosepline
        ...
        \begin{align*}
            ...
            \right\}
        \end{align*}
        \State Initialize a message queue $\scheduler=\{(\secsedge,\sedge) \wcols \secsedge \text{ has empty incoming nodes} \}$ \janina{What would that be in Fig 10?}
