...
The package consists in four subpackages in three layers of abstraction:
%\begin{figure}[t]
\begin{center}
...
We demonstrate the basic usage of the \tnreason{} package orienting on the Example~\ref{exa:propFormulaCoordinatewise}.
We first install (e.g. by \mintinline{shell}{pip install tnreason == 2.0.0}) and import the package by
\begin{minted}{python}
...
%\end{minted}
Orienting on \defref{def:tensor} the tensor instances in \mintinline{python}{shape} and \mintinline{python}{colors} arguments are \mintinline{python}{list} instances specifying the \mintinline{python}{int} dimension $\catdimof{\catenumerator}$ and a \mintinline{python}{str} identifier for $\catvariableof{\catenumerator}$.
...
\end{minted}
The slice iterator is an iterator over tuples \mintinline{python}{(val,posDict)}, which specify elementary tensors to be summed as follows.
