...
To match these demands, artificial intelligence has followed symbolic paradigms, to which probabilistic and logical approaches have been developed.
...
We in this work propose to fill the gap between the probabilistic, neural and logical paradigm with \emph{tensor networks}, in a framework called \tnreason{}. %~\cite{goessmann_tensor-network_2025}.
% Representation
...
We show that fundamental sparsity principles of neural and symbolic AI, such as conditional independence, existence of sufficient statistics and neural model decomposition, are equivalent to tensor network decompositions.
% Reasoning
...
The architecture consistent in two complementary subarchitectures, which serve the purpose of computation and activation, respectively.
The \emph{computation network} prepares auxiliary hidden variables with deterministic dependence on the main variables.
We interpret the network as a distributed computation scheme of functions describing the dependence, which decomposition is related to sparsity concepts of the corresponding functions. %is reflected in the computation.
...
Logical models emerge when the activation network is a boolean tensor, probabilistic exponential families when they are elementary positive valued tensors, and hybrid models in most general cases.
...
We present in section~\ref{sec:notation} the basic concepts and notation for categorical variables, tensors, and tensor networks, which will be applied in the following sections.
...
Section~\ref{sec:neurPar} is dedicated to the \emph{neural paradigm}, where we show that generic function decompositions have their analogous tensor networks.
...
In Section~\ref{sec:hln} we then present \emph{\HybridLogicNetworks{}} as an application of the unified tensor network formalism, which combine logical and probabilistic models.
We briefly discuss the implementation of these concepts in the \python{} package \tnreason{} in Section~\ref{sec:implementation}, before concluding the paper in section~\ref{sec:conclusion}.
