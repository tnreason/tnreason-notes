...
\begin{itemize}
    \item \janina{Manchmal schreiben wir $\shortcatvariables=(\catvariables)$ und manchmal $\shortcatvariables=\catvariables$. Ich bin pro $\shortcatvariables=(\catvariables)$.}
    \item \janina{Added dim in Ex. \ref{exa:cpFormat}. Correct? Check Fig \ref{fig:TTHypergraph}.}
    \item \janina{In Def. \ref{def:expFamily} $\Gamma=\mathbb{R}^p$?}
    \item \janina{In Alg. \ref{alg:treeBeliefPropagation}, I possibly undid some of the changes in the Ensure and return during a merge.}
    \item \janina{In Thm. \ref{the:treeBeliefPropagationExactness} correction?}
\end{itemize}
...
In Section~\ref{sec:notation} we introduce the basic concepts and notation for categorical variables, tensors, and tensor networks, which will be applied in the following sections.
...
In Section~\ref{sec:hln} we then present \emph{\HybridLogicNetworks{}} as an application of the unified tensor network formalism, combining logical and probabilistic models.
