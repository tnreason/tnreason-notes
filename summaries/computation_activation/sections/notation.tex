...
\begin{definition}[Tensor]
    ...
    \alex{We further abbreviate sets as $[\catorder]=\{0,\ldots,\catorder-1\}$, tuples of state indices by $\catindexof{[\catorder]}=\catindexof{0},\ldots,\catindexof{\catorder-1}$ and tuples of variables by $\catvariableof{[\catorder]}=\catvariableof{0},\ldots,\catvariableof{\catorder-1}$.}
\end{definition}
...
\begin{example}[Trivial Tensor]
    ...
    \alex{Maybe directly do the Dirac tensor (special case of $d=1$ is the trivial tensor).}
    The trivial tensor
    \begin{align*}
...
%\subsubsection{Tensor Product} \alex{As an example of contractions}
%
%
%
...
\begin{definition}[One-hot encodings]
    \label{def:onehotenc}
    \alex{Do it for factored directly}
...
\alex{We can also allow for variables taking values in infinite sets such as $\rr$.
Contractions are then performed by generic integrations with respect to e.g. the Lebesque measure.
}