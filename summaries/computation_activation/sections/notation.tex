...
In this section, we introduce our hypergraph-based tensor network formalism and define the architecture of \CompActNets{} based on this formalism.

\subsection{Tensors}
\label{sec:tensors}

Tensors are multiway arrays and a generalization of vectors and matrices to higher orders.
We will first provide a formal definition as real maps from index sets enumerating the coordinates of vectors, matrices, and higher-order tensors.
...
While this may look like syntactic sugar at this point, it will allow us to define contractions without further specification of axes, based on comparisons of shared variables.
...
\begin{example}[Delta Tensor]\label{exa:diracDelta}
    ...
    \begin{align}
        ...
        \end{cases} \, .
    \end{align}
    We will depict this tensor by black dots, which will sometimes appear as auxiliary elements in tensor network diagrams (see e.g. \figref{fig:contraction}).
...
In addition, we assign to each axis of the tensor the corresponding variable $\catvariableof{\atomenumerator}$:
\begin{center}
...
When an open variable $\catvariable$ does not appear in any tensor in a contraction, we define the contraction as a tensor product with the trivial tensor $\onesat{\catvariable}$.
To ease notation, we will often omit the set notation by brackets $\{\cdot\}$. % and specify the tensors to be contracted with the delimiter "," (see e.g. \exaref{exa:tensorProduct}).
...
\begin{example}[Tensor Product]\label{exa:tensorProduct}
...
\end{align*}
where $\catvariable,\headvariable$ are variables taking values in $[\cardof{\inset}]$ and $[\cardof{\outset}]$.
We present in \exaref{exa:madicRepresentation} index enumeration maps for summations in $\catdim$-adic integer representations.
%
...
\begin{align*}
    \maxgraph  = \big(\nodes,\{\nodes\}\big) \, .
\end{align*}
We therefore have for any graph $\realizabledistsof{\sstat,\graph}\subset\realizabledistsof{\sstat,\maxgraph}$.
