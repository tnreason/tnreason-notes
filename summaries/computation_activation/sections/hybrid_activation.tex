...
\begin{align*}
    ...
    \, .
\end{align*}
We notice that $\lossof{\hybridparam}=\infty$ if and only if there is a data point $\datindexin$ with
\begin{align*}
    \hlnformulaat{\shortcatvariables=\shortcatindices^{\datindex}}=0 \, ,
\end{align*}    
where the tuple $(\variableset,\headindexof{\variableset})$ denotes the hard logic part of the network, $t=(f_1,\dots,f_L)$, and
\begin{align*}
    \hlnformulawith =
...
If $\lossof{\hybridparam}<\infty$, we can rewrite the loss using the empirical mean vector $\datameanwith\in\parspace$, which is defined for $\selindexin$ as
\begin{align*}
