\begin{example}[Independent Boolean Variables]
    ...
    \begin{figure}
        ...
        The independencies are captured by the elementary hypergraph a), whose edges contain single nodes.
        The corresponding tensor $\probwith$ is then represented by a Markov Network on the elementary hypergraph, where each factor is the marginal distribution of the corresponding variable as visualized in b).}\label{fig:ELDecomposition}
    \end{figure}
\end{example}