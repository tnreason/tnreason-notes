\begin{example}[Continuation of \exaref{exa:madicRepresentation}]%[Computation of the sum of $\catdim$-adic integers]
    ...
    Let us now show how \algoref{alg:directedBeliefPropagation} can be exploited to compute by an efficient message passing algorithm the digits of the $\catdim$-adic sum.
    ...
    The initial messages queue consists then in the messages from the digit encoding.
    As sketched in \figref{fig:propagationMary} to each digit there are three messages (with the exception of the last being two), which can be scheduled in consecutive epochs $\messagesymbol^{(\catenumerator,[3])}$.
    %, namely
%    \begin{align*}
%        \scheduler =&
%        \left\{
...
%    \end{align*}
    ...
    We notice, that the hypergraph representing this instance is a tree and by \theref{the:treeBeliefPropagationExactness} also the message passing scheme of \algoref{alg:treeBeliefPropagation} is guaranteed to produce the exact contractions.
    ...
    We strengthen that the directed belief propagation \algoref{alg:treeBeliefPropagation} is exact even if the hypergraph fails to be a tree, provided that we have directed and boolean tensors..
