...
%    \end{align*}
    ...
    \begin{align*}
        ...
        \quad,\quad
        \indexinterpretationat{\shortcatindices} = \sum_{\catenumeratorin} \catindexof{\catenumerator} \cdot \catdim^{\catenumerator} \, .
    \end{align*}

    Let us now describe the sum of two $\catdim$-ary numbers as a decomposition hypergraph.
    The targeted composition function is
    \begin{align*}
        \exfunctionof{\graph,\catenumerator}(\shortcatindices,\tildecatindexof{[\catorder]})
        = \left\lfloor\frac{\left(\sum_{\seccatenumerator\in[\catorder]} (\catindexof{\seccatenumerator}+\tildecatindexof{\seccatenumerator}) \cdot \catdim^{\seccatenumerator}\right) \,\,\mathrm{mod}\,\, \catdim^{\catenumerator+1} }{\catdimof{\catenumerator}}\right\rfloor
    \end{align*}

    We build a decomposition hypergraph consistent in the $3\cdot \catorder$ nodes decorated by
    \begin{itemize}
        \item $\catdim$-ary representation of the first number: $\shortcatvariables$
        \item $\catdim$-ary representation of the second number: $\seccatvariableof{[\catorder]}$
        \item $\catdim$-ary representation of the sum: $\headvariableof{[\catdim+1]}$
        \item Auxiliary variables: $\thirdcatvariable_{[\catorder-1]}$
    \end{itemize}
    ...
    \end{align*}
    decorated by functions
    \begin{align*}
        + \defcols [2] \times [\catdim] \times [\catdim] \rightarrow [\catdim] \times [2]
        \quad,\quad
        +(\thirdcatindex,\catindex,\tildecatindex)
        = \left(\thirdcatindex + \catindex + \tildecatindex \,\,\mathrm{mod}\,\, \catdim,
              \left\lfloor\frac{\thirdcatindex + \catindex + \tildecatindex}{\catdim}\right\rfloor\right) \, .
    \end{align*}
    The function decorating the first hyperedge is the restriction of the first argument to $0$.
    ...
    \begin{figure}
        ...
        \begin{center}
            \begin{tikzpicture}[scale=0.35, thick]
                ...
                \node[anchor=center] (A) at (1,0) {\corelabelsize $\bencodingof{+}$};
