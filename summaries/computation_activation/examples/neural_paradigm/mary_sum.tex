\begin{example}[Sum of integers in $\catdim$-ary representation]
    \label{exa:marySum}
    ...
    \begin{align*}
        ...
        +(i,\tilde{i}) = i+\tilde{i} \,
    \end{align*}
    which have a $\catdim$-ary representation of length $\catorder$.
...
%    The $\catdim$-ary representation of length $\catorder$ encodes numbers $i\in[\catdim^{\catdim}]$ by indices $\shortcatindices$.
%    We understand these by index interpretation map
%    \begin{align*}
%        \indexinterpretation \defcols \facstates \rightarrow [\catdim^{\catorder}]
%        \quad,\quad
%        \indexinterpretationat{\shortcatindices}
%        = \sum_{\catenumeratorin} \catindexof{\catenumerator} \cdot \catdim^{\catenumerator} \, .
%    \end{align*}
%    Let us now describe the sum of two $\catdim$-ary numbers as a decomposition hypergraph.
    ...
    \end{align*}
%    where we denote for $\selindex\in[\catorder+1]$ the functions
%    \begin{align*}
%        \exfunctionof{\graph,\selindex} \defcols [\catdim^\catorder] \times [\catdim^\catorder] \rightarrow [\catdim^{\catorder+1}]
%        \exfunctionof{\graph,\selindex}(\shortcatindices,\tildecatindexof{[\catorder]})
%        = \left\lfloor\frac{
%            \left(
%                \sum_{\seccatenumerator\in[\catorder]} (\catindexof{\seccatenumerator}+\tildecatindexof{\seccatenumerator}) \cdot \catdim^{\seccatenumerator}
%            \right) \modspace\catdim^{\selindex+1} }{\catdimof{\selindex}}\right\rfloor
%    \end{align*}
...
%    \begin{itemize}
%        \item $\catorder+\catorder$ variables $\shortcatvariables,\seccatvariableof{[\catorder]}$ of dimension $\catdim$, representing the $\catdim$-ary decomposition of the input numbers.
%        \item $\catorder+1$ variables $\headvariableof{[\catdim+1]}$ of dimension $\catdim$, representing the $\catdim$-ary decomposition of the sum.
%        \item $\catorder-1$ auxiliary variables $\thirdcatvariableof{[\catorder-1]}$ of dimension $2$ representing carry bits.
%    \end{itemize}
    ...
    It is known that the composition of the local summations $\modsumsymbol$ is the global summation $\marysumsymbol$ of integers in $\catdim$-ary representation.
    ...
    \begin{align*}
        ...
        = \breakablecontractionof{
            &\{\bencodingofat{\modsumsymbol}{\headvariableof{0},\thirdcatvariableof{0},\catvariableof{0},\tildecatvariableof{0}}\} \cup \\
            &\{\bencodingofat{\modsumsymbol}{
            ...
            &\{\bencodingofat{\modsumsymbol}{\headvariableof{\catorder-1},\headvariableof{\catorder},\catvariableof{\catorder-1},\tildecatvariableof{\catorder-1},\thirdcatvariableof{\catorder-2}}\}
    ...
    \begin{figure}[t]
        \begin{center}
            \begin{tikzpicture}[scale=0.35, thick]
                ...
                \node[anchor=center] (A) at (1,0) {\corelabelsize $\bencodingof{\modsumsymbol}$};
            ...
            \end{tikzpicture}
        \end{center}
        \caption{Example of a decomposition hypergraph to the sum of integers (see \exaref{exa:marySum}).
        a) Hypergraph of directed edges $\edgeof{\catenumerator}$ for $\catenumeratorin$, each decorated by a integer summation $+$ preparing an index $\headvariableof{\catenumerator}$ of the resulting sum.
        b) Corresponding tensor network decomposition of the basis encoded composition function, which is the sum of integers in $\catdim$-ary representation.}
