\begin{example}[Continuation of \exaref{exa:studentHC}]
    \label{exa:studentBP}
    Let us now exemplify the Belief Propagation \algoref{alg:beliefPropagation} on the Markov Network in the student example (see \exaref{exa:studentHC}).
    ...
    Since the hyperedges are minimally connected, we can implement \algoref{alg:beliefPropagation} by a tree scheduler $\scheduler$:
    \begin{itemize}
        ...
        In our example, after exectution of $(\edgeof{2},\edgeof{0})$ the message $(\edgeof{0},\edgeof{1})$ is placed on $\scheduler$ and after exectution of $(\edgeof{1},\edgeof{0})$ the message $(\edgeof{0},\edgeof{2})$.
    \end{itemize}
    In this implementation \algoref{alg:beliefPropagation} terminates after $\cardof{\dirovedges}=4$ iterations of the $\mathrm{While}$ loop.
