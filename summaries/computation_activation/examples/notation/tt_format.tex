\begin{example}[The $\ttformat$ format]\label{exa:ttFormat}
    The Tensor-Train ($\ttformat$) (see \cite{oseledets_tensor-train_2011}) format corresponds in our notation to a hypergraph (see \figref{fig:TTHypergraph}) defined by
    \begin{itemize}
        ...
        \end{align*}
        each decorated by a tensor of order 3 (respectively 2 for $\catenumerator\in\{0,\catorder-1\}$).
    \end{itemize}
    \begin{figure}
        \begin{center}
...
%    \coordinate[label=below:$\hdots $] (A) at (9,0.35);
        ...
        b) Corresponding tensor-network on the edges of the hypergraph. \janina{Shouldn't the edges in a) connect two of the I and only one X? And I only goes to d-2 in both?}}\label{fig:TTHypergraph}
    \end{figure}
