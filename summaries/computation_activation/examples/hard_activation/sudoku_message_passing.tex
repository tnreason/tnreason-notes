\begin{example}[Constraint Propagation for the Sudoku of \exaref{exa:sudokuEntailment}]
    ...
    \begin{itemize}
        ...
        This entailment step is performed by $4$ consecutive messages (see $\messagesymbol^{0,[4]}$ in \figref{fig:contractionPropagationSudoku}) involving decomposition cores of the position constraint in the position $(r_0,r_1,c_0,c_1)=(0,0,0,0)$.
        ...
        This entailment step is performed by $7$ consecutive messages (see $\messagesymbol^{1,[7]}$ in \figref{fig:contractionPropagationSudoku}) involving decomposition cores of the constraint to the number $i=3$ in the first row $(r_0,r_1)=(0,0)$.
    ...
    This entailment step is performed by four consecutive messages (see $\messagesymbol^{2,[4]}$ in \figref{fig:contractionPropagationSudoku}) along the directions $(\catvariableof{0,1,0,1,2},\decvariableof{0,:,0,:,2})$, $(\catvariableof{0,0,0,0,2},\decvariableof{0,:,0,:,2})$, $(\catvariableof{0,0,0,1,2},\decvariableof{0,:,0,:,2})$ and $(\decvariableof{0,:,0,:,2},\catvariableof{0,1,0,0,2})$.
...
\begin{figure}[t]
    \begin{center}
        \begin{tikzpicture}[scale=0.35,thick]
            ...
            \drawvariabledot{4}{-3}
            \draw (4,-3) -- (3.25,-3);
            \draw (4,-5)--(4,-1);% node[midway,right] {\colorlabelsize $\catvariableof{0,0,0,0,2}$};

            \draw (-1,-7) rectangle (1,-5);
            \node[anchor=center] (A) at (0,-6) {\corelabelsize $\hypercoreof{0}$};
            \drawvariabledot{0}{-3}
            \draw (0,-3) -- (-0.75,-3);
            \draw (0,-5)--(0,-1);% node[midway,right] {\colorlabelsize $\catvariableof{0,0,0,0,2}$};

            \draw (-5,-7) rectangle (-3,-5);
            \node[anchor=center] (A) at (-4,-6) {\corelabelsize $\hypercoreof{2}$};
            \draw (-4,-5)--(-4,-3); % node[midway,right] {\colorlabelsize $\catvariableof{0,0,1,0,2}$};

            \drawvariabledot{-4}{-3}
            \draw (-4,-3) -- (-4.75,-3);
            %\node[anchor=center] (A) at (-8,-2) {\corelabelsize $\ddots$};
            ...
            \draw[\newmessagecolor,dashed, <-] (-1.5,-1) to [bend right = 30] (-1.5,-2.75);
            \node[\newmessagecolor,anchor=center] (A) at (-2,-0.5) {\colorlabelsize $\messagesymbol^{1,4}$};

            \draw[\newmessagecolor,dashed, <-] (-1.5,-3.25) to [bend right = 30] (-1.5,-5);
            \node[\newmessagecolor,anchor=center] (A) at (-2.75,-4) {\colorlabelsize $\messagesymbol^{1,3}$};

            \draw[\newmessagecolor,dashed, <-] (2.5,-1) to [bend right = 30] (2.5,-2.75);
            \node[\newmessagecolor,anchor=center] (A) at (2,-0.5) {\colorlabelsize $\messagesymbol^{1,6}$};

            \draw[\newmessagecolor,dashed, <-] (2.5,-3.25) to [bend right = 30] (2.5,-5);
            \node[\newmessagecolor,anchor=center] (A) at (1.25,-4) {\colorlabelsize $\messagesymbol^{1,5}$};
            ...
            \draw (12,-5)--(12,-2.5); % node[midway,right] {\colorlabelsize $\catvariableof{0,1,0,1,2}$};
            ...
            \draw[\newmessagecolor,dashed, <-] (13.5,-3.25) to [bend right = -30] (13.5,-5);
            \node[\newmessagecolor,anchor=center] (A) at (14.75,-4) {\colorlabelsize $\messagesymbol^{0,2}$};

            \draw[\newmessagecolor,dashed, <-] (13.5,-1) to [bend right = -30] (13.5,-2.75);
            \node[\newmessagecolor,anchor=center] (A) at (14.75,-0.75) {\colorlabelsize $\messagesymbol^{0,3}$};
    ...
    \end{center}
    \caption{
        Tensor network decomposition of three rules for the $2^2\times2^2$ Sudoku rules, namely to the number $3$ appearing once in the $(0,0)$-square (above), the number $3$ appearing once in the $(0,0)$-row (below left) and a unique number appearing at the $(0,1,0,1)$-position (below right).
        The tensor network therefore showcasts $3$ out of $2\cdot2^4=128$ rules in the Sudoku knowledge base (see \exaref{exa:sudokuDecomposition}).
        The evidence of the number $3$ assigned at the position $(0,0,1,0)$ is sketched on by a basis vector $\textcolor{\concolor}{\tbasis}$ on the left side, and the number $2$ assigned at position $(0,1,0,1)$ analogously on the right side.
        During Constraint Propagation \algoref{alg:constraintPropagation} on the hypergraph of Sudoku rules and evidence (see \exaref{exa:sudokuMessagePassing}), this evidence leads to non-trivial messages $\messagesymbol$, which propagate the constraints by partial entailment steps and imply that $\textcolor{\probcolor}{\catvariableof{0,1,0,0,2}}$ is true, i.e. that at the position $(0,1,0,0)$ the number $3$ needs to be assigned.
    %    Depiction of the first reasoning step in \exaref{exa:sudokuEntailment} by messages passed in the $\mathrm{While}$ loop of the .
